---
layout: about
title: about
permalink: /

profile:
  align: right
  image: fc_grad25.jpeg
  image_circular: false

links:
  - label: email
    url: mailto:fionacai@stanford.edu
  - label: google scholar
    url: https://scholar.google.com/citations?user=9h7KpuAAAAAJ&hl=en
  - label: linkedin
    url: https://www.linkedin.com/in/fiona-cai/
  - label: twitter
    url: https://twitter.com/fionaxcai
  - label: github
    url: https://github.com/fionaxc

papers:
  - title: Uncertainty Inclusive Contrastive Learning for Leveraging Synthetic Images
    authors: Fiona Cai, Emily Mu, John Guttag
    venue: CVPR 2024 Synthetic Data for Computer Vision Workshop
    url: https://openreview.net/forum?id=wNKOQVcMDS
  - title: Improving the Efficiency of Clinical Trial Recruitment Using an Ensemble Machine Learning to Assist With Eligibility Screening
    authors: Tianrun Cai, Fiona Cai, Kumar P. Dahal, Gabrielle Cremone, Ethan Lam, Charlotte Golnik, Thany Seyok, Chuan Hong, Tianxi Cai, Katherine P. Liao
    venue: ACR Open Rheumatology, 2021
    url: http://dx.doi.org/10.1002/acr2.11289
  - title: "PheProb: Probabilistic Phenotyping Using Diagnosis Codes to Improve Power for Genetic Association Studies"
    authors: Jennifer A Sinnott, Fiona Cai, Sheng Yu, Boris P Hejblum, Chuan Hong, Isaac S Kohane, Katherine P Liao
    venue: Journal of the American Medical Informatics Association, 2018
    url: http://dx.doi.org/10.1093/jamia/ocy056
  - title: On-Body Piezoelectric Energy Harvesters through Innovative Designs and Conformable Structures
    authors: Sara V. Fernandez, Fiona Cai, Sophia Chen, Emma Suh, Jan Tiepelt, Rachel McIntosh, Colin Marcus, Daniel Acosta, David Mejorado, Canan Dagdeviren
    venue: ACS Biomaterials Science & Engineering, 2021
    url: http://dx.doi.org/10.1021/acsbiomaterials.1c00800

projects:
  - title: alphaDOM
    description: reinforcement learning agent trained to play the board game dominion
    url: https://github.com/fionaxc/alphaDOM

news: false
latest_posts: false
selected_papers: false
social: false
---

Hi! I'm a second year PhD student in the Department of Biomedical Data Science at Stanford University where I am fortunate to be advised by [Emily Alsentzer](https://emilyalsentzer.com/) and [Serena Yeung-Levy](https://ai.stanford.edu/~syyeung/). My current research interests lie at the intersection of machine learning, computer vision, and representation learning and their applications to healthcare and medical imaging. My work is supported by the [NSF Graduate Research Fellowship](https://www.nsfgrfp.org/).

I previously completed my B.S and M.Eng in Computer Science at MIT, where I was advised by [Prof. John Guttag](https://caml.csail.mit.edu/). In the past, I have worked on clinical informatics problems using traditional statistical machine learning approaches for tasks like clinical trial recruitment and probabilistic phenotyping.

Outside of research, I love to travel and I'm a huge fan of strategy and card games — ask me anything about polytopia, catan, dominion, poker, guan dan, da guai lu zi :)
